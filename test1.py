...
def load_registered_faces():
    ...
    for image_name in os.listdir("register_images"):
        if image_name.endswith(".jpg"):
            ...
            # Assign a unique label to each person based on the filename
            name = image_name[:-4]  # Remove .jpg
...
def recognize_faces():
    # Load the registered faces and names
    registered_faces, labels, label_map = load_registered_faces()
    recognizer = cv2.face.LBPHFaceRecognizer_create()

    # Train the recognizer
    recognizer.train(registered_faces, labels)

    # Invert the label_map for reverse lookup (label -> name)
    ...
    while True:
        ...
        for (x, y, w, h) in faces:
            ...
            if confidence < 70:  # Adjusted confidence threshold for better accuracy
